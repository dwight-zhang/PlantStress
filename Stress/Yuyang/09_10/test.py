...
def Cmatrix(rho0, rho1,rho1b,rho2,rho2b):
  Cmat = Yg*(1*(globals()['E00']+globals()['E11']) + v*(globals()['E10']+globals()['E01']) + (1-v)/2*globals()['E22']) 
  return Cmat
...
u.Set(CoefficientFunction(( 1, 0, 0)))
...
for i in range(10):
    ...
    norm = 1 / (InnerProduct(du, res))
    du *= norm 
    print ("norm =", norm, "err^2 =", InnerProduct(du, res))
    u.vec.data -= du.data 
